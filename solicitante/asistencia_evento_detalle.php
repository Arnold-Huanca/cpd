...
$ERROR = ''; 
  ...
  $asistencia_evento    = new Asistencia_evento($id);
   ...
   $funcionario= new Funcionario($asistencia_evento ->funcionario_id);
...
// combo box subarea
 ...
 //echo $usuario->nombre;
  if (isset($_POST['tarea']) && $_POST['tarea'] == 'registrar' && isset($_POST['token']) && $_SESSION['register'] == $_POST['token'])
			...
			$tipo_archivo = $_FILES['archivo']['type'];
				...
				if ($tamanio > $max_length) {
					...
					if ($upload -> UploadFile($name)){
						 $asistencia_evento->archivo = "uploads/".$name.".".$upload->ext;
					}
				}
			}
		}
    ...
    mysql_query("COMMIT");
   $ir = "Location: asistencia_evento.php?menus=mostrar&funcionario_id= $asistencia_evento->funcionario_id";
    header($ir);
     exit();
...
$TEMPLATE_TOSHOW = 'solicitante/asistencia_evento_detalle.tpl';
